...
class HomePageRepository {
  static Future<NotifierState<List<GetCategories>>> getCategories() async {
    return (await ApiService<List<GetCategories>>()
            .getCallOnlyList("/user/category/list",
                // hasToken: true,
  ...
  static Future<NotifierState<GetProductsBySearch>> getProductsBySearch(
      {required String query, required String categoryId}) async {
    return (await ApiService<GetProductsBySearch>()
            .getCall("/user/product/search/$query/$categoryId",
                onReturn: (response) => logResponse(response),
                getDataFromResponse: (data) {
  ...
  static Future<NotifierState<GetProductsBySearch>> getSavedList(
      {required String query, required String categoryId}) async {
    return (await ApiService<GetProductsBySearch>()
            .getCall("/user/shopping_list/lists",
                onReturn: (response) => logResponse(response),
                getDataFromResponse: (data) {
                  return GetProductsBySearch.fromJson(data);
                }))
  ...
  static Future<NotifierState<String>> addToList(
      {required AddProductRequest request}) async {
  ...
  static Future<NotifierState<String>> deleteSavedList(
      {required int listId}) async {
    return (await ApiService<String>()
            .deleteCall("/user/shopping_list/delete/$listId",
                hasToken: true,
                onReturn: (response) => logResponse(response),
                getDataFromResponse: (data) {
                  return data["message"];
  ...
  static Future<NotifierState<Map<String, dynamic>>> createOrder(
      {required CreateOrderRequest orderRequest}) async {
  ...
  static Future<NotifierState<String>> loadCoupon(
    ...
    return (await ApiService<String>().postCall("/user/coupons/load",
            ServiceRequest(serviceRequest: {"code": coupon}),
            // hasToken: true,
            onReturn: (response) => logResponse(response),
            getDataFromResponse: (data) {
  ...
  static Future<NotifierState<Map<String, dynamic>>> getContact() async {
    return (await ApiService<Map<String, dynamic>>()
            .getCall("/user/site-data/site_data",
                onReturn: (response) => logResponse(response),
                getDataFromResponse: (data) {
                  ...
                  SessionManager.setFacebook(
                      data["contact_data"].toString().split(";")[1]);
                  SessionManager.setTwitter(
                      data["contact_data"].toString().split(";")[2]);
                  SessionManager.setContactEmail(
                      data["contact_data"].toString().split(";")[4]);
                  SessionManager.setContactPhone(
                      data["contact_data"].toString().split(";")[3]);
                  return data;
                }))
        .toNotifierState();
  }
  ...
  static Future<NotifierState<List<GetQuickGuide>>> getQuickGuide() async {
    return (await ApiService<List<GetQuickGuide>>()
            .getCallOnlyList("/user/quickguide/list",
                onReturn: (response) => logResponse(response),
                getDataFromResponse: (data) {
