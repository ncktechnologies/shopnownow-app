...
class _CheckOutState extends ConsumerState<CheckOut> {
              ...
              children: [
                ...
                Row(
                  children: [
                    ...
                    XBox(kPadding),
                    Icon(
                      isShow ? Icons.keyboard_arrow_up : Icons
                          .keyboard_arrow_down,
                      size: 25,
                      color: kPrimaryColor,
                    )
                ...
                SessionManager.getEmail() != null ? YBox(0) : TextInputNoIcon(
                  text: emailAddress,
                  validator: (val) {
                    if (val!.isEmpty) {
                      return emptyField;
                  ...
                  children: [
                    ...
                    PaymentRow(
                      ...
                      subText: (double.parse(subTotalCalculation()) *
                          double.parse((int.parse(widget.tax!.replaceAll(
                              ".00", "")) / 100).toString())).toStringAsFixed(
                          2),
                    ),
                    //ABC456
                    PaymentRow(
                                        ...
                                        if (formKey.currentState!.validate()) {
                                          ...
                                          CreateOrderRequest(
                                            ...
                                            tax: (double.parse(
                                                ...
                                                .toInt(),
                                            status: "pending",
                                            deliveryInfo: addressController
                                                .text,
                                            paymentType: "card",
                ...
                Column(
                  ...
                  children: [
                    ...
                    PaymentRow(
                      text: "$tax(${widget.tax?.replaceAll(".00", "")}%)",
                      subText: subTotalCalculation() == "0" ? "0" : (double
                          .parse(subTotalCalculation()) * double.parse((int
                          .parse(widget.tax!.replaceAll(".00", "")) / 100)
                    ...
                    PaymentRow(
                        text: total,
                        subText: double.parse(
                            finalAmountToBePaid(subTotalCalculation()))
                            .toStringAsFixed(2)),
                    YBox(kMediumPadding),
                    Row(
                      crossAxisAlignment: CrossAxisAlignment.start,
                              ...
                              Consumer(builder: (context, ref, _) {
                                var buttonWidget = LargeButton(
                                    ...
                                    //     (double.parse(subTotalCalculation()) * double.parse((int.parse(widget.tax!.replaceAll(".00", "")) / 100).toString())))).toString()}"
                                        : "Pay ₦ ${finalAmountToBePaid(
                                        subTotalCalculation()).startsWith("-")
                                        ? finalAmountToBePaid(
                                        subTotalCalculation()).substring(1)
                                        : finalAmountToBePaid(
                                        subTotalCalculation()) }",
                                    onPressed: () {
                                      if (isChecked) {
  ...
  String subTotalCalculation() {
    ...
    if (totalAmount > int.parse(widget.band!.bulkDiscountAmount!)) {
      discount2 =
      (totalAmount * (int.parse(widget.band!.bulkDiscountPercentage!) / 100));
    }

    subtotal = couponAmount > totalAmount ? 0 : totalAmount -
        (discount1 + discount2 + couponAmount);
    ...
    if (double.parse(subTotalCalculation()) >
      ...
      if (double.parse(SessionManager.getWallet()!.replaceAll(".00", "")) >
          double.parse(((double.parse(subTotalPrice) +
              (double.parse(subTotalPrice) * double.parse(
                  (int.parse(widget.tax!.replaceAll(".00", "")) / 100)
                      .toString())))).toString())) {
        amount = "0";
      } else {
        amount =
            (int.parse(
                SessionManager.getWallet()!.replaceAll(".00", "") ?? "0") -
