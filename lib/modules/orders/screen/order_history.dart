...
class _OrderHistoryState extends ConsumerState<OrderHistory> {
  ...
  Widget build(BuildContext context) {
    return InitialPage(
      ...
      child:ref.watch(orderProvider).when(done: (data){
        if(data == null){
                              ...
                              child: Column(
                                ...
                                children: [
                                  ...
                                  Row(
                                    children: [
                                      Text(
                                        DateFormat("dd MMM, ").add_jm().format(data.orders![index].createdAt!),
                                  ...
                                  Container(
                                    ...
                                    decoration: BoxDecoration(
                                        borderRadius: BorderRadius.circular(
                                            kSmallPadding),
                                        color:data.orders![index].status == "delivered" ? kSuccess : kLight800),
                                    child: Text(
                                      data.orders![index].status!.toTitleCase(),
                                      style: textTheme.headlineMedium!.copyWith(
                                          fontSize: 10, color: data.orders![index].status == "delivered" ? kToastColor2: kYellow),
                                    ),
                                  ),
                                ],
                              ),
                            ),
        data.orders![index].status == "delivered"  ?  Container(
                              padding: const EdgeInsets.symmetric(
      ...
      }, loading: ()=> const SpinKitDemo(), error: (val)=> YBox(0))
    ));
  }
}
